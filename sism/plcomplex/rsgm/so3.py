# taken from https://github.com/gcorso/DiffDock/blob/001c4fa46e37ac878acf7f85f67908952c5e067f/utils/so3.py
# also helpful:
# https://github.com/jasonkyuyim/se3_diffusion/blob/53359d71cfabc819ffaa571abd2cef736c871a5d/data/igso3.py
# https://github.com/jasonkyuyim/se3_diffusion/blob/53359d71cfabc819ffaa571abd2cef736c871a5d/data/so3_diffuser.py
...
# Get the directory where this so3.py file is located
...
# Define the file paths relative to the current file's directory
omegas_file = current_dir / ".so3_omegas_array4.npy"
cdf_file = current_dir / ".so3_cdf_vals4.npy"
score_norms_file = current_dir / ".so3_score_norms4.npy"
exp_score_norms_file = current_dir / ".so3_exp_score_norms4.npy"

if omegas_file.exists():
    _omegas_array = np.load(omegas_file)
    _cdf_vals = np.load(cdf_file)
    _score_norms = np.load(score_norms_file)
    _exp_score_norms = np.load(exp_score_norms_file)
else:
    print(
        "Preprocessing for the SO(3) sampling and score computations, truncated infinite series are computed and then cached to memory, therefore the precomputation is only run the first time the repository is run on a machine"
    )
    ...
    # Save files in the same directory as this script
    np.save(omegas_file, _omegas_array)
    np.save(cdf_file, _cdf_vals)
    np.save(score_norms_file, _score_norms)
    np.save(exp_score_norms_file, _exp_score_norms)
